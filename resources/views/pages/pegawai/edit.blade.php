...
@section('content')
    <div class="max-w-2xl mx-auto bg-slate-800 p-6 rounded-2xl shadow-lg">
        <h2 class="text-xl font-semibold mb-6">Edit Pegawai</h2>

        <form action="{{ route('pegawai.update', $pegawai) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="number" name="umur" id="umur" value="{{ old('umur', $pegawai->umur) }}"
                    class="w-full px-3 py-2 rounded-lg bg-slate-700 text-white border border-slate-600 focus:ring focus:ring-blue-500">
                @error('umur')
                    <p class="text-red-400 text-sm">{{ $message }}</p>
                @enderror
            ...
            <div>
                ...
                <textarea name="alamat" id="alamat" rows="3"
                    class="w-full px-3 py-2 rounded-lg bg-slate-700 text-white border border-slate-600 focus:ring focus:ring-blue-500">{{ old('alamat', $pegawai->alamat) }}</textarea>
                @error('alamat')
                    <p class="text-red-400 text-sm">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="golongan_id" id="golongan_id"
                    ...
                    @foreach ($golongan as $g)
                        <option value="{{ $g->id }}"
                            {{ old('golongan_id', $pegawai->golongan_id) == $g->id ? 'selected' : '' }}>
                            {{ $g->nama }}
                        </option>
                    @endforeach
                </select>
